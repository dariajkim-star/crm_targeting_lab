...
> 반대로 말한다.
>
> **(2026-07-22 2차 개정 — 스토리 3-0 코드리뷰)** 위 ②가 원안의 보증 문구와 충돌한다는 것이 리뷰에서
> 드러났다. 원안 Rule은 "이탈점수·SHAP은 **동일 아티팩트**"라고 적었는데, OOF 전환 이후 `churn_score`를
> 만드는 것은 번들에 담기지 않는 **폴드 모델 k개**이고 SHAP이 설명하는 것은 **최종 모델**이다. 실물
> 증거: `tests/churn/test_explain.py`가 SHAP 정합 검증의 기준을 저장 점수에서 최종 모델의 자체 점수로
> 바꿔야 했다 — 테스트가 먼저 이 불일치에 부딪힌 것이다.
> 개정 내용: 보증 범위를 **동일 학습 실행(run)**으로 좁히고, `churn_score`는 "그 아티팩트의 OOF 형제"로
> 명시한다. 폴드 모델을 번들에 포함하는 안은 기각했다 — 아티팩트가 5배가 되고, 그 모델들은 점수를
> 만들고 버려지며, SPEC 비목표(서빙 없음)에 비추어 보관 근거가 없다.
> **약속을 낮춘 것이 아니라, 처음부터 지킬 수 없었던 약속을 기록한 것이다**(CAP-5 3차 개정·AD-11
> 소급 반영과 같은 관례). 서명: 불확실을 확실로 세탁하지 않는다.
...
- **Rule:**
  - `churn_prob_calibrated`·SHAP 값·요인 top5는 **동일 학습 아티팩트**에서 산출한다. SHAP은 `03_train_churn`에서만 계산하고 후속 단계는 **읽기만** 한다(재계산 금지 — 재학습 금지만으로는 부족).
  - **`churn_score`는 그 아티팩트의 OOF 형제다** — 같은 데이터·같은 seed·같은 하이퍼파라미터로 폴드별 학습된 모델들이 만들지만, 번들에 담긴 최종 모델과 **같은 객체는 아니다**. 따라서 "점수와 설명이 동일 모델"은 `churn_score`에 대해서는 **성립하지 않으며, 성립시킬 수도 없다**(OOF의 정의상 각 고객의 점수는 그 고객을 뺀 모델에서 나온다). AD-5가 실제로 보증하는 것은 **동일 학습 실행(run) 소속**이지 동일 모델 객체가 아니다.
