...
- **AD-5 아티팩트 정체성**: `churn_prob_calibrated`·SHAP·요인 top5는 동일 학습 아티팩트에서 산출(`churn_score`는 그 아티팩트의 **OOF 형제** — 스토리 3-0 2차 개정), `models/churn_model.meta.json`의 `artifact_id` 불일치 시 `05_marts` 즉시 실패.
...
>
> **근거**
> - **Epic 3은 Epic 1에만 의존한다** — 소비하는 것은 이탈 점수/확률(`churn_score`·`churn_prob_calibrated`)과 `customer_value`뿐이고
...
**Given** `churn_prob_calibrated`와 SHAP 해석이 함께 소비될 때
**When** 정체성을 검증하면
...
> ### 스토리 3-0 추가 (2026-07-22) — 이 문서의 FR에서 유도되지 않은 스토리
>
> **`3-0-oof-scores-platt-calibration`**: 이탈 점수를 **out-of-fold**로 바꾸고, 금액 계산에 쓰는
> 확률에 **Platt 보정**을 적용하며, 컬럼을 목적별로 분리한다.
>
> ```
...
> ```
>
> **왜 여기 없던 스토리인가**: FR12(기대절감액 산식)는 `P(이탈)`을 입력으로 요구하지만, 그 값이
> **보정된 확률이어야 한다**는 것을 이 문서도 SPEC도 말하지 않았다. 에픽1 회고 액션 **A2**와 스토리
> 3-1 **외부 리뷰 M3**가 합쳐져 드러난 요구사항이다 —
> ① `churn_prob`는 in-sample·미보정이라 곱셈의 크기로 쓰면 기대절감액이 계통적으로 부푼다
> ② 보정 방식에 따라 3-1 분면 판정이 움직일 수 있다(isotonic은 58명, Platt은 0명 — 실측).
>
> **새 요구사항을 만드는 것이 아니라**, FR12가 성립하기 위한 **전제**를 뒤늦게 명시하는 것이다.
> FR 커버리지 매핑은 바뀌지 않는다.
>
> **순서**: 3-1이 닫힌 **뒤**, 3-2 **앞**. 3-0이 3-1 리포트 수치를 갱신한다(OOF 전환 시 분면이
> 446/2086/4621/2974 → 443/2089/4624/2971로 이동).
>
> 실측 근거·대가·P1 선례: `docs/implementation-artifacts/deferred-work.md` 「A2 결정」절
...
> **(2026-07-23 분할 — 에픽3 회고 B1, daria 승인)** 4-1이 에픽3 미결 부채 9건의 수렴점이라 3-3급 하중이 되어 **4-1a/4-1b로 분할**한다(1-6a/b·3-3 선례, John "스토리 크기→리뷰 폭증" 교훈 예방). **4-1a**(고객 마트 조립·CLIENTNUM 조인·정규 스키마) = 아래 AC 전부 + 함정4 라벨 조인 + AD-12 정의 고정. **4-1b**(계약 좁히기·오용 방지·리포트 경로) = `target_priority` 3축 CLIENTNUM 계약 좁히기 · `random_baseline` 모집단 정체성 · `budget=0` UX · `churn_score`/`churn_prob_calibrated` 오용 방지 표기 · `customer_value` 호출부 재가중 보호 · 세션 리포트 커밋 경로. **FR15 커버리지는 두 스토리 합으로 충족**되며 매핑은 바뀌지 않는다. 상세: `docs/implementation-artifacts/epic-3-retro-2026-07-23.md` §6, `sprint-status.yaml`.
...
**When** 스키마를 확인하면
**Then** 모든 컬럼이 `name | dtype | 단위 | nullable | 산출 모듈 | 정의 1줄`로 망라 열거되고, `quadrant_official`·`target_priority`·`segment_id`·**`churn_score`**·**`churn_prob_calibrated`**·`customer_value`·`threshold_official_*`가 포함된다(AD-2·AD-3·AD-5)
> **(2026-07-22 정정 — 스토리 3-0)** 원안은 단일 `churn_prob`를 요구했다. AD-5 개정으로 컬럼이 순위용·확률용 둘로 나뉘었으므로 **둘 다 열거한다** — `churn_prob_calibrated`만 실으면 `threshold_official_*`(컷)가 함께 실려도 분면을 검산할 수 없고, 2×2는 이 프로젝트의 중심 논리라 최종 산출물에서 재현 가능해야 한다. **이것은 새 요구가 아니라 개정된 AD-5를 반영한 것이며 FR 커버리지 매핑은 바뀌지 않는다.** 오용 방지를 어떻게 표기할지는 4-1 스토리가 결정한다(`deferred-work.md` 「4-1 입력」).
