...
- [ ] **T1 — 취득 경로 확정** (AC1): frames / 벌크 / 주석 벌크 3종 URL·구성 확인. 주석 도달
      가능 여부 예/아니오 판정. 응답 캐시 + 취득 meta(시각·SHA-256).
- [ ] **T2 — peer 축 구축** (AC2): `submissions` 메타에서 SIC·filer status 수집 → 셀 크기 분포 산출.
