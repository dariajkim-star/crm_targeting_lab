...
- [ ] **T1 — 조립 헬퍼 신설** (AC1, AC2)
  - [ ] `crm/marts/customers.py`에 순수 함수 `build_customer_mart(bankchurners, features, scored) -> pd.DataFrame` 작성. 세 프레임을 `CLIENTNUM` 라벨 조인, 인덱스=CLIENTNUM. 집합 불일치·중복 시 `ValueError`(명확한 메시지).
  - [ ] 조립 프레임에서 각 축을 뽑아 crm 함수 호출: `customer_value(bankchurners_indexed)` · `assign_quadrant(churn_score, value)` (+ `quadrant_thresholds` → `threshold_official_*`) · `expected_saving(churn_prob_calibrated, value)` · `target_priority(expected_saving, value, clientnum)`. `segment_id`는 features에서 그대로.
  - [ ] **AD-1 확인**: 이 모듈은 `crm.ltv`를 절대 import하지 않는다(import-graph 테스트가 자동 강제 — `_LANE_A` 소속으로 판정돼 `_LANE_B` 금지). segment+churn+campaign import는 허용.
  - [ ] **AD-11 확인**: `customer_value` 출력을 재계산·재가중하지 않고 그대로 소비. `Total_Trans_Amt`를 이 모듈에서 명명하지 않는다(value.py만 소유 — `find_value_recomputation_violations` fail-closed).
- [ ] **T2 — `pipelines/05_marts.py` 신설** (AC1, AC4, AC5, AC7)
  - [ ] `main(input_paths, output_paths)` 하나만. ≤40행. 허용 호출은 `crm.*`·pandas read/write·logging뿐.
  - [ ] 입력 읽기 → `artifact_id` 게이트(불일치 즉시 실패) → `build_customer_mart` → 행수 assert(10,127) → non-nullable null 검사 → 스키마 순서로 컬럼 정렬 → `crm/common/atomic.py`로 원자적 쓰기 + meta.json.
  - [ ] LTV 레인 미수행을 docstring에 명시(에픽2 동결).
- [ ] **T3 — `marts/mart_customers.schema.md` 정규 작성** (AC3)
  - [ ] 전 컬럼 `name | dtype | 단위 | nullable | 산출 모듈 | 정의 1줄`. 기대 행 수 10,127 명시. `target_priority` 정의 AD-12대로 고정. `expected_saving`·`customer_value`는 "가정"(성공률 0.30·비용 5.0) 라벨 병기.
  - [ ] 컬럼 순서 = CSV 직렬화 순서(단일 출처).
- [ ] **T4 — 테스트** (AC2, AC3, AC4, AC6)
  - [ ] 스키마 일치: `set(df.columns) == schema.columns` + dtype (AD-2 필수 3종 ①).
  - [ ] 결정론 바이트 동일: `tests/test_determinism` 또는 기존 결정론 스위트에 마트 CSV 추가 (필수 ②).
  - [ ] import-graph 레인 격리·의존 방향: 새 `crm/marts/` 포함 (필수 ③, AD-1·AD-9).
  - [ ] **함정4 회귀 테스트(행동 기반)**: 세 소스를 일부러 다른 행 순서로 섞은 픽스처를 조립기에 넣어 (a) CLIENTNUM 조인이 올바른 정합을 내고 (b) 위치 결합이었다면 나왔을 오합계가 나오지 않음을 단언. 이것이 3-3이 밟은 +37.2% 사고의 회귀 가드다.
  - [ ] 행수 보존 assert 검증. 조립기 집합 불일치·중복 시 실패 검증.
