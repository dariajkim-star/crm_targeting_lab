...
**방식**: 3레이어 병렬(Blind/Edge/Auditor) → 파티 판정(daria 전권 위임). **Auditor 판정: CONCERNS** — AC2 불충족 포함, 구조 수정 필요.

**핵심 결함(3자 교차 확정)과 처분**:
- [x] **F1/H2/E2 (Major) — AD-3 ② "다른 탭" 위반**: create-story가 탭2(공식+시나리오) 설계로 ②를 조건부 완화, 사양서가 자기 위반. **처분: 시나리오 시트를 전용 탭5로 분리**(공식 4탭 + 시나리오 1탭 — FR17의 4탭은 공식 뷰 구성으로 정합화). ②를 문자 그대로 충족. *출처는 dev가 아닌 create-story 층 결함 — 스토리 AC1 T1의 "탭2(공식+시나리오)"가 원인이었음을 기록.*
- [x] **H3/E3 (Major) — ④ 충족 불능**: `threshold_official_*`가 필드 표에서 탭1 전용이라 시나리오 탭 병기 불가 → 탭5 배정 + 테스트가 탭 칸을 파싱해 5 포함 단언.
- [x] **F2 (Major) — 테스트가 정확히 ②만 미검증**: 4항목 전부 + epics 원문 문구("현재 임계값과 공식 임계값을 병기") 단언으로 재작성.
- [x] **H1/F3 (High) — 금지 조항 3 자기모순**: "성공률·비용만 허용"이 자기 시트의 예산 파라미터를 금지 → 취지로 교정("분면 정의를 바꾸는 파라미터 금지, 예산·성공률·비용 허용") + 단언 추가.
- [x] **E1/M2 (High) — 슬라이스 종료 앵커 -1 → 전문 검색 퇴화**: `_slice` 헬퍼(양 앵커 존재 단언) 도입, raw slice 전폐. `_section`도 라인 앵커 정규식으로(E7).
- [x] **M1/E3 — 필드 표 탭 칸 본문 불일치·미검증**: 표 전면 정정(1:1 정합) + 탭 칸 유효성 테스트.
- [x] **E4/E5/M5 — 산식 정합**: 파라미터 허용 값 명세(정수·최소 0·단계 500), 1-base 명시, `_BUDGET_TOL` 괴리 정직 각주(정수 예산에서 동치 0 보장). 전부 테스트 단언.
- [x] **M3 — AC3 미검증**: 머리말 슬라이스 테스트 신설(역할 경계·스크린샷 규칙).
- [x] **M4 — "금액 전용" 라벨 vs 축 사용 상충**: 용도 칸 정정 + churn_score 숨김 근거에 축 담당 명시.
- [x] **F4/F5/E6/E9/L2 (Minor)**: 탭별 원본 경로·캡션 규약 전역화(머리말 조항+테스트)·앵커 표 분면 라벨+골든 단언·스텁 비결합 단언·deferred-work 문법 복원.
- **기각**: L3(포섭 테스트 — 실패 메시지 품질로 존치), L1(done 즉시 처리로 무의미화), E8(실패 방향 안전).

**교훈 기록**: 문서-계약 테스트의 3대 함정이 한 스토리에서 전부 실증됨 — ①검증 목록에서 빠진 항목이 곧 표류한 항목 ②종료 앵커 소실 시 절-한정이 전문 검색으로 퇴화 ③표(단일 출처)의 미파싱 칸은 출처가 아니다. 셋 다 헬퍼·단언으로 봉인.
...
- 2026-07-24: 코드리뷰(3레이어+파티) 반영 — **시나리오 전용 탭5 분리**(AD-3 ② 문자 충족, 공식 4탭+시나리오 1탭), 금지 조항 3 자기모순 교정, 필드 표 탭 칸 1:1 정합+검증, 슬라이스 견고화(_slice/_section), 산식 정합 각주(1-base·_BUDGET_TOL·파라미터 정의역), 테스트 10→16건. 385→391 passed(회귀 0), 마트·코드 바이트 불변 유지.
