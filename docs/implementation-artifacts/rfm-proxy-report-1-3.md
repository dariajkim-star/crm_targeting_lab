...
- **DQ2 (부분 해소, 과잉주장 금지)**: 02는 `verify_inputs`(producer/config) +
  `is_output_stale`(자기 stage·config drift + **직접** 입력 콘텐츠) 2단 게이트를 통과해야
  재계산한다. **닫지 못한 것**(리뷰 High-2): ① 전이적 staleness — 입력이 바이트 동일하나 그
  원천이 바뀌고 하위 stage만 재실행되는 경우(05만 도는 대표 시나리오는 오케스트레이터 DAG
  검증이 있어야 잡힌다) ② 순수 코드 변경(config 무변) ③ 산출물 자체 변조. 모두
  deferred-work.md에 근거와 함께 기록. "DQ2 완전 해소"라고 말하지 말 것.
