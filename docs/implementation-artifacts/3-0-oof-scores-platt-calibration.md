...
**전건 종결(2026-07-22)**: patch 15건 적용, decision 4건은 파티 라운드에서 결론(아래 각 항목 말미).
테스트 313 → **338 passed**, 회귀 0.

- [x] [Review][Decision] **freshness 게이트가 컬럼 계약을 캐시 키에 담지 않는다** — 3-0 이전 산출물이 남은 환경에서 `crm/config.py`가 불변이므로 `config_hash`도 불변 → 구 artifact_id 3자가 서로 일치 → `outputs_share_identity` True → stage 03 전체가 return. 결과적으로 **옛 in-sample `churn_prob`가 살아남고 `churn_score`는 생성되지 않는다.** `artifact_id`는 "무엇을 직렬화했는지"(bare model vs bundle)를 기록하지 않아 번들 전환도 감지 못한다. 선택지: (a) 스키마/컬럼 계약을 `config_hash`에 편입 (b) 산출물 컬럼 존재를 게이트 조건에 추가 (c) 3-0 전환은 1회성으로 보고 수동 삭제 안내만. 근거: `crm/churn/freshness.py:36-37,205-207` · `pipelines/03_train_churn.py:23-26`
- [x] [Review][Decision] **번들 정체성이 "점수를 만든 모델"을 담지 않는다 — AD-5 보증 문구의 사정거리** — `artifact_id`는 `{최종 모델, 보정기}`를 해시하지만 저장된 `churn_score`는 **번들에 없는 k개 폴드 모델**의 산출물이다. `artifact.py`가 스스로 내건 목적("a churn_score and an explanation of it must not be presented together unless they describe the same model")이 문자 그대로는 더 이상 성립하지 않는다. 함정 4의 (ii)안 채택 결과이고 AD-5·deferred-work에 근거는 기재됐으나, **보증 문구 자체는 좁혀지지 않았다.** `test_explain.py`가 저장 점수를 버리고 `result.model.predict_proba`로 갈아탄 것이 이 균열의 실물 증거다. 선택지: (a) AD-5 보증 문구를 "설명과 보정은 동일 아티팩트, 점수는 그 아티팩트의 OOF 형제"로 축소 개정 (b) 폴드 모델까지 번들에 포함 (c) 현행 유지 + 문서 보강
- [x] [Review][Decision] **`epics.md` 마트 스키마가 아직 단일 `churn_prob`를 요구한다 — 4-1이 개정 전 계약대로 만든다** — AD-5는 컬럼을 둘로 나눴는데 에픽은 하나를 요구한다. 이건 과거 기록이 아니라 **살아 있는 요구사항**이다. 결정 필요: `mart_customers`에 (a) `churn_prob_calibrated`만 (b) 둘 다 (c) `churn_score`만. 근거: `docs/planning-artifacts/epics.md:579` (부수적으로 :61 · :106 · :339도 개정 전 AD-5 서술)
- [x] [Review][Decision] **`LogisticRegression()` 기본 L2(`C=1.0`)가 조용히 걸려 있다 — 교과서 Platt이 아니다** — 주석은 "No regularisation sweep ... Platt scaling is a one-parameter (plus intercept) correction by definition"이라 적어 무정규화인 것처럼 읽히지만 sklearn 기본값은 `penalty="l2", C=1.0`이다. 계수를 0쪽으로 수축시켜 보정 곡선을 평탄화하며, 표본 크기·점수 스케일에 따라 보정량이 달라진다. 선택지: (a) `C=1e10`으로 사실상 무정규화 (b) 현행 유지 + 주석을 사실대로 정정. 실측 평균 0.1607은 어느 쪽이든 충족. 근거: `crm/churn/calibrate.py:109-112`

> **결정 결과 (2026-07-22, 파티 라운드 — Mary·Paige·John·Sally·Winston·Amelia)**
>
> - **D1 → (b) 산출물 컬럼을 게이트 조건에 추가.** `artifact.py::outputs_are_current` 신설(1-6b
>   `outputs_share_identity`는 **재구현 없이 합성만**), stage 03은 import·호출을 제자리 교체해 **40행 유지**.
>   라운드 중 등급 조정 2회: Mary가 "대부분 경로는 KeyError로 터진다"며 한 칸 낮췄고, Winston이
>   "04/05가 먼저 도는 순서에서만 조용하다"며 그 한 경로를 남겼다. 사용자 확인으로 **가정된 위험이
>   아니라 관측된 위험**으로 확정 — `data/`를 유지한 채 브랜치를 넘나드는 것이 이 리포의 정상 작업 방식이다.
>   회귀 테스트: `test_pre_3_0_schema_outputs_force_a_rerun`(옛 스키마 + 정상 정체성 → 재실행 강제).
> - **D2 → (a) 보증 문구 축소 개정.** AD-5 Rule을 "동일 학습 **실행(run)** 소속"으로 좁히고
>   `churn_score`를 "그 아티팩트의 **OOF 형제**"로 명시. `verify_artifact_identity` docstring도 동반 정정.
>   (b) 폴드 모델 번들 포함은 기각 — 아티팩트 5배, 점수를 만들고 버려지는 객체, SPEC 비목표(서빙 없음).
>   Paige 판정: *"깨진 계약은 고치면 되지만 넓은 약속은 아무도 깨진 줄 모른다."* 약속을 낮춘 것이
>   아니라 처음부터 지킬 수 없었던 약속을 기록한 것(CAP-5 3차·AD-11 소급 선례).
> - **D3 → (a) 마트에 두 컬럼 모두 + 스키마 문서에 `금지` 칸 신설.** 결정적 논거는 Sally —
>   `threshold_official_*`(컷)을 실으면서 점수를 빼면 **자물쇠만 주고 열쇠를 안 주는 것**이고, 2×2가
>   중심 논리인 이상 최종 산출물에서 검산 가능해야 한다. John의 반론(공개 마트에서 컬럼 오선택)은
>   Mary가 올린 실측 **+19.0%**(3-2 함정)로 흡수 — 오용 크기를 알면 경고를 구체적으로 쓸 수 있다.
>   층 분리: **마트는 감사 가능성, 대시보드(4-3)는 오용 방지.** `epics.md`는 **두 컬럼 열거만 정정**하고
>   (개정된 AD-5의 반영 — FR 커버리지 매핑 불변), **`금지` 칸·층 분리는 AC로 승격시키지 않고 4-1 시점으로
>   내렸다**(`deferred-work.md` 「4-1 입력」). 초안에서 AC를 늘렸다가 되돌린 것 — 3-0이 "새 요구사항을
>   만들지 않는다"고 명시해 놓고 리뷰 결정으로 AC를 늘리면 그 원칙을 우리 손으로 깬다(사용자 지적).
>   stale 3곳(:61 AD-5 서술 · :106 Epic3 의존 · :339 정체성 검증)도 정정.
> - **D4 → (a) 주석만 사실대로 정정.** 코드 결함이 아니라 **서술 결함**(계수 +10.4, 평균 0.1607 정확).
>   Sally 관통: *"반만 맞는 문장이 제일 오래 살아남는다"* — "sweep 안 했다"는 참, "정규화 없다"는 거짓.
>   `C=1e10` 또는 config 상수화는 **기각이 아니라 deferred** — 3-2가 기대절감액 기준선을 이 확률 위에
>   고정하기 직전이라 지금 움직이면 그 기준선을 흔든다. 분면은 어느 쪽이든 불변(단조성이 지킨다).
...
| 2026-07-22 | 코드리뷰 decision 4건 종결(파티): D1 게이트에 컬럼 계약 추가(`outputs_are_current` 신설, stage 40행 유지, 관측된 위험으로 확정) · D2 AD-5 보증을 '동일 run'으로 축소 개정 · D3 마트 2컬럼 + 스키마 `금지` 칸(오선택 +19.0%) · D4 주석 정정(C 상수화는 deferred). **337 → 338 passed.** Status → done |
