...
| 날짜 | 변경 |
|---|---|
| 2026-07-22 | 스토리 3-0 구현: OOF 점수(`churn_score`) + Platt 보정(`churn_prob_calibrated`) 투트랙, {model,calibrator} 번들 정체성, AD-5 개정. 분면 443/2089/4624/2971로 이동(원인은 보정 아닌 OOF 전환), PR-AUC 0.9508 불변, 2회 실행 완전 동일. 313 → 330 passed. Status → review |
