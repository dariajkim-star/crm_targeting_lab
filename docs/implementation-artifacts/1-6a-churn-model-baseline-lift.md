...
```
baseline 로지스틱 PR-AUC 0.4286 | XGBoost 0.8044 | lift +87.7% (목표 +15% 상회)
...
- **AC1 충족**: baseline 로지스틱(class_weight=balanced) + XGBoost(scale_pos_weight·`random_state`·`n_jobs=1`·
  `tree_method="hist"`). StratifiedKFold seed 주입 5-fold **PR-AUC** 병행. 생성자 params로 결정론·불균형 주입 검증.
- **AC2 충족**: baseline 대비 XGBoost PR-AUC **리프트 +87.7%** 수치 제시. +15%는 판정 아닌 서술 — 상회했으나
...
- **테스트**: 170 → **183 passed** (+13: churn 13), 회귀 0.
...
| 2026-07-21 | 스토리 1-6a 구현: model.py·artifact.py·03_train_churn·xgboost 3.3.0. baseline 0.4286→XGBoost 0.8044(+87.7%), 결정론·누수배제. 세그먼트별 churn_prob가 1-5 이탈률과 정합. 170 → 183 passed, 회귀 0. Status → review |