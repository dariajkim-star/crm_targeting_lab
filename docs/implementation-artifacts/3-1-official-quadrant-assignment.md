...
재리뷰 번들(`scratch/rereview-3-1.*`)을 준비했으나 **2라운드를 돌리지 않고 done 처리한다**(사용자 결정).

**근거**: 1라운드가 **High 0 · 반려 0**이었고 지적 9건이 전부 **계약·검증·문구** 계열이었다. 판정
수치는 처음부터 끝까지 불변이다(컷 0.126842 / 3899.0, 분면 446/2086/4621/2974). 지적은 모두 재현
후 패치했고 제안된 변이 M7~M13을 전부 테스트로 편입했다.

**감수하는 것**: 1라운드 수정이 새 구멍을 냈을 가능성은 검증되지 않았다 — 특히 `QuadrantAssignment`
도입과 검증 함수 3개(`_validate_rule`·`_validate_axis`·`_validate_pair`)의 호출 순서다. 번들은
`scratch/`에 남아 있으므로 필요해지면 그대로 돌릴 수 있다. **3-0이 이 리포트 수치를 재산출하며
`matrix.py`를 실제로 다시 호출하므로**, 그 시점에 계약이 한 번 더 실사용으로 검증된다.
