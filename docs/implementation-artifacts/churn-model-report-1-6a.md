...
`churn_score`(구 `churn_prob`)는 미래 시점 확률이 아니라 **현재 스냅샷에서의 이탈 위험 점수**다. 이 한계는 보정 여부와 무관하게 그대로 인계된다(AD-6).
...
- **결정론(AD-7)**: XGBoost `random_state=42`·**`n_jobs=1`**·`tree_method="hist"` 고정. CV는
  ...
  03 2회 실행 `churn_score` 동일은 **회귀 테스트로 고정**(`test_stage_output_is_deterministic_across_two_runs`).
...
### 점수 컬럼 — **스토리 3-0에서 해소됨** (2026-07-22 갱신)

> **이 절의 원문은 다음과 같았다**: *"최종 모델은 전체 고객으로 학습한 뒤 같은 고객을 채점하므로
> `churn_prob`는 out-of-fold가 아닌 **in-sample 점수**이고, `scale_pos_weight` 재가중 때문에
> **보정된 확률이 아니다**(8피처 실측: 평균 0.195 vs 실제 이탈률 0.161). 컬럼명은 스파인(AD-5)이
> 고정한 것이라 유지하되 순위 신호로만 사용할 것. calibration은 평가하지 않았다."*
>
> **스토리 3-0이 두 문제를 모두 닫았다**(에픽1 회고 A2 + 3-1 외부 리뷰 M3):

| | 이전 | 현재 |
|---|---|---|
| 점수 | `churn_prob` — in-sample, 미보정 | **`churn_score`** — **out-of-fold**, 순위 전용 |
| 확률 | 없음 | **`churn_prob_calibrated`** — Platt 보정, 확률 전용 |
| 평균 | 0.195 (실제 0.161) | score 0.1946 / **calibrated 0.1607 = 실제와 일치** |

- **컬럼명이 유지되지 않았다** — 원문은 "스파인이 고정한 이름이라 유지"라고 적었으나, 3-0이
  **AD-5를 개정**했다. 확률이 아닌 값에 `prob`을 남기면 문서가 아무리 경고해도 컬럼명이 반대로 말한다.
- **아래 표의 PR-AUC는 바뀌지 않았다.** `pr_auc_cv()`는 처음부터 홀드아웃 채점이었고, in-sample이었던
  것은 **저장 컬럼 하나뿐**이었다. 즉 이 리포트의 헤드라인 수치는 그때도 정직했다.
- 아래 세그먼트 평균 비교는 여전히 방향 정합의 참고일 뿐 **독립 검증·calibration 증거가 아니다**.

## 파이프라인 정합성 (참고)

산출된 점수의 **세그먼트별 평균**은 1-5의 세그먼트별 실제 이탈률과 정합한다:

| segment_id | 평균 점수 (3피처) | 평균 점수 (8피처, in-sample 당시) | 1-5 실제 이탈률 |
|---|---|---|---|
